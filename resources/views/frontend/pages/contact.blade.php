...
@php
    ...
    $formTitle = $sections['form_title'] ?? 'Ready to Get Started?';
    $formText = $sections['form_text'] ?? ($page->content ?? 'Nullam varius, erat quis iaculis dictum, eros urna varius eros, ut blandit felis odio in turpis. Quisque rhoncus, eros in auctor ultrices,');
    $phoneLabel = $sections['phone_label'] ?? 'Call Us 7/24';
    $emailLabel = $sections['email_label'] ?? 'Make a Quote';
    $locationLabel = $sections['location_label'] ?? 'Location';
    $videoUrl = $sections['video_url'] ?? 'https://www.youtube.com/watch?v=Cn4G2lZ_g2I';
    $videoImage = !empty($sections['video_image'])
        ? asset('storage/'.$sections['video_image'])
        : asset('assets/img/video.jpg');
    $mapSrc = $settings->map_embed_url
...
@include('frontend.partials.breadcrumb', [
    'title' => $page->breadcrumb_title ?? $page->title ?? 'Contact Us',
            ...
            <div class="row g-4 align-items-center">
                <div class="col-lg-6">
                    <div class="contact-left-items">
                        <div class="contact-info-area-2">
                            ...
                            <div class="contact-info-items mb-4">
                                ...
                                <div class="content">
                                    <p>{{ $emailLabel }}</p>
                                    <h3>
                                        <a href="mailto:{{ $settings->email ?: 'info@example.com' }}">{{ $settings->email ?: 'Infotech@gmail.com' }}</a>
                                    </h3>
                                </div>
                            </div>
                            ...
                            <div class="contact-info-items border-none">
                                ...
                                <div class="content">
                                    <p>{{ $locationLabel }}</p>
                                    <h3>{{ $settings->address ?: '4517 Washington ave.' }}</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="video-image">
                            ...
                            <div class="video-box">
                                <a href="{{ $videoUrl }}" class="video-btn ripple popup-video">
                ...
                <div class="col-lg-6">
                    <div class="contact-content">
                        <h2>{{ $formTitle }}</h2>
                        <p>{{ strip_tags((string) $formText) }}</p>

                        <form action="{{ route('contact.submit') }}" method="POST" class="contact-form-items" id="contact-form">
                            @csrf
                            <div class="row g-4">
